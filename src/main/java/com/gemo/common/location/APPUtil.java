package com.gemo.common.location;
...
public class APPUtil {
	...
	public static String[] paks = new String[]{"com.baidu.BaiduMap",
			"com.autonavi.minimap"};//, "com.gemo.bookstadium"};
	...
	public static void startNative_Gaode(Context context,Location loc){
		...
		try {
			Intent intent = new Intent("android.intent.action.VIEW",
					android.net.Uri.parse("androidamap://navi?sourceApplication=场馆通&poiname=目的地&lat="+loc.getLat()+"&lon="+loc.getLng()+"&dev=1&style=2"));
...
//		return "http://api.map.baidu.com/direction?origin=latlng:"+loc1.getStringLatLng()+"|name:"+loc1.getAddress()+"&destination=latlng:"+loc2.getStringLatLng()+"|name:"+loc2.getAddress()+"&mode=driving&src=重庆快易科技|CC房车-车主";
		return String.format("http://api.map.baidu.com/marker?location=%s,%s&title=目的地&content=体育馆&output=html", loc2.getLat(), loc2.getLng());
	}
